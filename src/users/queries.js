...
const getUsers = 'SELECT u.* AS svapp_user, i.url AS image_url, un AS user_name FROM svapp_user u INNER JOIN image i ON i.image_id = u.image_id INNER JOIN user_name un ON un.name_id = u.name_id';
...
const getUserById = 'SELECT u.* AS svapp_user, i.url AS image_url, un AS user_name, vi.* AS verification_image FROM svapp_user u INNER JOIN image i ON i.image_id = u.image_id INNER JOIN user_name un ON un.name_id = u.name_id INNER JOIN verification_image vi ON vi.id = u.verification_image WHERE svapp_user_id = $1'
